...
export async function POST(request: Request) {
  ...
  try {
    const validatedData = SignInWithOAuthSchema.parse({ provider, providerAccountId, user });
    ...
    if (!existingAccount)
      await Account.create([{ userId: existingUser._id, provider, providerAccountId }], { session });
