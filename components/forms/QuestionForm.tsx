...
const QuestionForm = () => {
  ...
  const handleCreateQuestion = async (data: AskQuestionFormValues) => {
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, field: { value: string[] }) => {
    if (e.key === "Enter") {
      ...
      if (tagInput && tagInput.length < 15 && !field.value.includes(tagInput)) {
        ...
        form.setError("tags", { type: "manual", message: "Tag length should be less than 15 characters" });
      else if (field.value.includes(tagInput)) form.setError("tags", { type: "manual", message: "Tag already exists" });
    }
  };
          ...
          render={({ field }) => (
            <FormItem className="flex w-full flex-col gap-3">
              ...
              <FormControl>
                <div>
                  <Input
                    className="paragraph-regular dark:bg-dark-400 no-focus light-border-2 text-foreground min-h-14 border bg-amber-50"
